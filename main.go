...
func main() {
	...
	flag.Parse()
	if *maxSlots < 1 || *maxSlots > 254 {
		log.Fatal("-max 必须为 1-254")
	}
	...
	if *publicIP != "" && net.ParseIP(*publicIP).To4() == nil {
		log.Fatal("-ip / FANOUT_PUBLIC_IP 必须是 IPv4 地址")
	}
	...
	srv := newWebServer(StripBasePath(secureRequests(pool.exportAuth(auth.Wrap(mux)))))
	tlsConfig, fingerprint, err := managementTLS(*workDir)
	if err != nil {
		log.Fatal(err)
	}
	srv.tlsConfig = tlsConfig
	log.Printf("管理证书 SHA256: %s", fingerprint)
	...
	log.Printf("管理界面: https://<本机IP>%s%s/", webCfg.listenAddrString(), currentBasePath())
